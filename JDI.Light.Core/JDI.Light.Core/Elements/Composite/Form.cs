...
using ObjectExtensions = JDI.Light.Core.Extensions.ObjectExtensions;
...
namespace JDI.Light.Core.Elements.Composite
{
    public class Form<T> : UIElement, IForm<T>
    {
        ...
        {
            Fill(ObjectExtensions.FieldsAsDictionary(entity));
        }
        ...
        {
            Check(ObjectExtensions.FieldsAsDictionary(entity));
        }
        ...
        {
            return Verify(ObjectExtensions.FieldsAsDictionary(entity));
        }
        ...
        {
            var fields = ObjectExtensions.GetFieldsOfType(obj, typeof(IButton)).ToList();
            if (!fields.Any())
                fields = ObjectExtensions.GetFieldsOfType(obj, typeof(IWebElement)).ToList();
            switch (fields.Count)
            {
                case 0:
